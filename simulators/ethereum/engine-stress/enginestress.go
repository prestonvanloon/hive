...
// Perform executePayload with valid parentHash/blockHash but colliding blockNumber.
func badBlockNumberStress(t *TestEnv) {
	if !t.WaitForPoSSync() {
	...
	for badBlockInsertCount > 0 {
		...
		// Reconstruct Payload with the invalid blocknumber (minus 1)
		customNumber := t.CLMock.LatestPayloadBuilt.Number - 1
		newPayload, err := customizePayload(&t.CLMock.LatestPayloadBuilt, &CustomPayloadData{Number: &customNumber})
		if err != nil {
			t.Fatalf("FAIL (%v): Unable to customize payload: %v", t.TestName, err)
		}
		resp, err := t.Engine.EngineExecutePayloadV1(t.Engine.Ctx(), newPayload)
		if err == nil {
			t.Fatalf("FAIL (%v): Invalid payload did not result in error: %v", t.TestName, resp)
		}
