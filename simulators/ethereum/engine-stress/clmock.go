...
func (cl *CLMocker) checkTTD() {
	...
	if td.TotalDifficulty.ToInt().Cmp(terminalTotalDifficulty) >= 0 {
		...
		time.AfterFunc(time.Second*1, cl.stressMinePOSBlock)
		return
	}
	time.AfterFunc(tTDCheckPeriod, cl.checkTTD)
}
	...
	for {
		...
		if dummyPayloadExecutionCount > 0 {
			...
			for i := 0; i < dummyPayloadExecutionCount; i++ {
				...
				for i := 0; i < common.HashLength; i++ {
					dummyRandom[i] = byte(rand.Uint32())
				}
				dummyPayload, err := customizePayload(&cl.LatestPayloadBuilt, &CustomPayloadData{Random: &dummyRandom})
				if err != nil {
					unlockAll(cl)
				...
				for i, resp := range cl.broadcastExecutePayload(dummyPayload) {
					if resp.Status != "VALID" {
						fmt.Printf("Error while sending dummy payload (%v): %v\n", i, resp)
					}
				}
			}
